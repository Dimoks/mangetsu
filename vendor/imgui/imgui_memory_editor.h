...
// - v0.59 (2025/07/07): added extended selection features, copy functionality, address input options, search panel, and UTF-8 display.
//
// TODO:
        ...
        while (clipper.Step())
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
            {
                ...
                if (OptShowAscii)
                {
                    ...
                    if (mouse_off_x >= 0.0f && mouse_off_x < s.PosAsciiEnd - s.PosAsciiStart)
                    {
