#
# src: https://stackoverflow.com/a/69239861/
#
  param(
  ...
  if ($LiteralPath -in $currDirs) {
    Write-Output "`r`n`t`"$LiteralPath`"`r`nis already present in the persistent $(('user', 'machine')[$isMachineLevel])-level path.`r`n"
    return
  }
  ...
  Write-Output "`r`n`t`"$LiteralPath`"`r`n`nwas successfully appended to the persistent $(('user', 'machine')[$isMachineLevel])-level Path. You must restart the Explorer.exe process or your PC for this change to take effect system-wide."
